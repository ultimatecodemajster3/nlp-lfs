...
# model_path = "Miranda/t5-small-train"
model_name ="saved"
...
def _initialize():
    ...
    if tokenizer is None or model is None:
        ...
        # model = AutoModelForSeq2SeqLM.from_pretrained(model_path, force_download=True)
...
def translate(text: str):
    ...
    _log_msg(model.config, True)

    sentences = preprocess(text=text)
    
    _log_msg("Sentences:" + str(sentences), True)
    _log_msg("Sentences length:" + str(len(sentences)), True)
    # sentences = text
    ...
    for sentence in sentences:
        tok = tokenizer.encode(sentence, return_tensors="pt",padding=True)
        translated  = model.generate(tok, max_length=300, num_beams=4, 
